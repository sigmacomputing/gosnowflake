...
func TestSubmitQuerySync(t *testing.T) {
	postMock := func(_ context.Context, _ *snowflakeRestful, _ *url.URL, _ map[string]string, _ []byte, _ time.Duration, _ currentTimeProvider, _ *Config) (*http.Response, error) {
		dd := &execResponseData{}
		er := &execResponse{
			Data:    *dd,
			Message: "",
